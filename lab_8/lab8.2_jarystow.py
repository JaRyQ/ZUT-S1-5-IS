...
class Chessboard:
    def __init__(self, n=4, board_state=None):  # inicjalizacja
        self.size = n  # do w
        self.board = np.zeros(
            (self.size), dtype=np.int) if board_state is None else board_state
...
class PMX:
    ...
    def cross(self, begin, end):
        ...
        print("corrected", self.X, self.Y, end="\n")
    ...
    def __correctOffspring(self, chessboard, mappingList, begin, end):  # TODO można poprawić
        for key, value in mappingList.items():
            try:
                ...
                try:
                    index = chessboard.getIndexOfValue(
                        key, end, self.n)
                    chessboard.setValueOfIndex(value, index)
                except ValueError:
                    pass
...
class EvolutionAlgorithm:
    ...
    def __generateInitialPopulation(self):
        ...
        for i in range(populationSize):
            ...
            chessboard = Chessboard(
                n=self.__chessboardSize, board_state=boardState)
    ...
    def doTheEvolution(self):  # main function
        ...
        evaluateScroes = self.__evaluate(
            currentPopulation, self.__populationSize)
        ...
        while(self.__isGenerationUnderLimit() and self.__isSolutionNotFound(currentPopulation, BestIndex)):
            ...
            self.__mutation()
            evaluateScroes = self.__evaluate(
                self.__newPopulation, self.__populationSize)
            BestIndex = self.__getIndexOfBestPopulation(evaluateScroes)
    ...
    # zwróć liczbę ataków między hetmanami na szachownicy
    def __evaluate(self, currentPopulation, size):
    ...
    def __selection(self, currentPopulation):  # selekcja turniejowa
        newPopulation = []
        for i in range(populationSize):
            firstOpponentIndex, secondOpponentIndex = self.__drawIndexes(
            ...
            if firstOpponentIndex != secondOpponentIndex:
                ...
                winner = firstOpponent if self.__isFirstTheWinner(
                    firstOpponent, secondOpponent) else secondOpponent
